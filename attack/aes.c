#include <stdint.h>

#include "aes.h"

static const unsigned char aes_sbox[256] = {
    
};

static const unsigned char aes_sbox_inv[256] = {
    
};

static unsigned char gdouble(const unsigned char x)
{
    return (x << 1) ^ (x & 0x80 ? 0x1b : 0);
}

void aes_addkey(unsigned char *a, const uint64_t k0, const uint64_t k1)
{
    for (int k = 0; k < 16; k++) {
        a[k] ^= (unsigned char)((k < 8 ? k0 : k1) >> ((k % 8) * 8));
    }
}

void aes_subbyte(unsigned char *a, const unsigned char *sbox)
{
    for (int i = 0; i < 16; i++) {
        a[i] = sbox[a[i]];
    }
}

void aes_shiftrow(unsigned char *a, const int inv)
{
    unsigned char t;

    if (inv) {
        t = a[1]; a[1] = a[13]; a[13] = a[9]; a[9] = a[5]; a[5] = t;
        t = a[3]; a[3] = a[7]; a[7] = a[11]; a[11] = a[15]; a[15] = t;
    } else {
        t = a[1]; a[1] = a[5]; a[5] = a[9]; a[9] = a[13]; a[13] = t;  
        t = a[3]; a[3] = a[15]; a[15] = a[11]; a[11] = a[7]; a[7] = t;
    }

    t = a[2]; a[2] = a[10]; a[10] = t; t = a[6]; a[6] = a[14]; a[14] = t;
}

void aes_mixcol(unsigned char *a)
{
    unsigned char a_1[4], a_2[4], a_3[4];

    for (int j = 0; j < 16; j += 4) {
        for(int i = 0; i < 4; i++) {
            a_1[i] = a[j+i];
            a_2[i] = gdouble(a_1[i]);
            a_3[i] = a_2[i] ^ a_1[i];
        }
        a[j]   = a_2[0] ^ a_3[1] ^ a_1[2] ^ a_1[3];
        a[j+1] = a_1[0] ^ a_2[1] ^ a_3[2] ^ a_1[3];
        a[j+2] = a_1[0] ^ a_1[1] ^ a_2[2] ^ a_3[3];
        a[j+3] = a_3[0] ^ a_1[1] ^ a_1[2] ^ a_2[3];
    }
}

void aes_mixcol_inv(unsigned char *a)
{
    unsigned char a_1[4], a_2[4], a_4[4], a_8[4], a_9[4], a_b[4], a_d[4], a_e[4];

    for (int j = 0; j < 16; j += 4) {
        for(int i = 0; i < 4; i++) {
            a_1[i] = a[j+i];
            a_2[i] = gdouble(a_1[i]);
            a_4[i] = gdouble(a_2[i]);
            a_8[i] = gdouble(a_4[i]);
            a_9[i] = a_8[i] ^ a_1[i];
            a_b[i] = a_8[i] ^ a_2[i] ^ a_1[i];
            a_d[i] = a_8[i] ^ a_4[i] ^ a_1[i];
            a_e[i] = a_8[i] ^ a_4[i] ^ a_2[i];
        }
        a[j]   = a_e[0] ^ a_b[1] ^ a_d[2] ^ a_9[3];
        a[j+1] = a_9[0] ^ a_e[1] ^ a_b[2] ^ a_d[3];
        a[j+2] = a_d[0] ^ a_9[1] ^ a_e[2] ^ a_b[3];
        a[j+3] = a_b[0] ^ a_d[1] ^ a_9[2] ^ a_e[3];
    }
}

void aes(unsigned char *a, const uint64_t k0, const uint64_t k1)
{
    aes_subbyte(a, aes_sbox);
    aes_shiftrow(a, 0);
    aes_mixcol(a);
    aes_addkey(a, k0, k1);
}

void aes_inv(unsigned char *a, const uint64_t k0, const uint64_t k1)
{
    aes_addkey(a, k0, k1);
    aes_mixcol_inv(a);
    aes_shiftrow(a, 1);
    aes_subbyte(a, aes_sbox_inv);
}
